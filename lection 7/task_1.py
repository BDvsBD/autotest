...
class Segment:
    ...
    def x_axis_intersection(self):
        if self.y1 * self.y2 < 0:
            ...
            x_intersection = self.x1 + (self.x2 - self.x1) * (0 - self.y1) / (self.y2 - self.y1)
            if min(self.x1, self.x2) <= x_intersection <= max(self.x1, self.x2):
                return True
        return False
    ...
    def y_axis_intersection(self):
        if self.x1 * self.x2 < 0:
            ...
            y_intersection = self.y1 + (self.y2 - self.y1) * (0 - self.x1) / (self.x2 - self.x1)
            if min(self.y1, self.y2) <= y_intersection <= max(self.y1, self.y2):
                return True
        return False
