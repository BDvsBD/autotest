...
class PersonInfo:
    ...
    def new_salary(self):
        letters_dic = {}
        for subdivision in self.subdivision:
            for letter in subdivision:
                if letter in letters_dic:
                    letters_dic[letter] += 1
                else:
                    letters_dic[letter] = 1
        top_3 = sorted(letters_dic.values())
        letters_sum = top_3[-1] + top_3[-2] + top_3[-3]
        return 1337 * self.age * letters_sum
