...
def everything_for_your_cat(cats_data):
    ...
    for cat in cats_data:
        cat_name, cat_age, owner_name, owner_surname = cat
        owner_key = owner_name, owner_surname
        if owner_key in owners_dict:
            owners_dict[owner_key].append((cat_name, cat_age))
        else:
            owners_dict[owner_key] = [(cat_name, cat_age)]
    for owner, cats in owners_dict.items():
        owner_name = owner[0]
        owner_surname = owner[1]
        cats_info = ''
        for cat in cats:
            cat_name = cat[0]
            cat_age = cat[1]
